...
def load_volfile(
    ...
    Parameters:
        ...
        ret_affine: Additionally returns the affine transform
...
def load_labels(arg):
    ...
    Parameters:
        arg: Path to folder containing label maps, string for globbing,
            or a list of these.
...
def resize(array, factor, batch_axis=False):
    """
    Resizes an array by a given factor. This expects the input array
...
def get_surface_pts_per_label(total_nb_surface_pts, layer_edge_ratios):
    """
    Gets the number of surface points per label, given the total
...
def sdt_to_surface_pts(
    ...
    # can't just correct by surface_pts_upsample_factor because of
