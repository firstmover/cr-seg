...
def slices(
    ...
    for _si, slice_in in enumerate(slices_in):
        if len(slice_in.shape) != 2:
            assert (
                len(slice_in.shape) == 3 and slice_in.shape[-1] == 3
...
def flow_legend(plot_block=True):
    ...
    plt.ylim(-2, 2)
    plt.quiver(
        x, y, u, v, color=colormap(norm(colors)), angles="xy", scale_units="xy", scale=1
    )
    plt.show(block=plot_block)
...
def flow(
    ...
    plot_block=True,  # option to plt.show()
    scale=1,
):  # note quiver essentially draws quiver length = 1/scale
    """
    ...
    if indexing == "ij":
        for si, _slc in enumerate(slices_in):
...
def pca(pca, x, y, plot_block=True):
    ...
    y_err = y_hat - y
    y_rel_err = y_err / np.maximum(
        0.5 * (np.abs(y) + np.abs(y_hat)), np.finfo("float").eps
    )
